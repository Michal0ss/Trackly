...
def login(form_data: OAuth2PasswordRequestForm = Depends()):
    db = SessionLocal()
    try:
        # OAuth2PasswordRequestForm exposes `username`, so we use it as email.
        db_users = db.query(models.Users).filter(models.Users.email == form_data.username).first()

        if not db_users or not db_users.password:
            raise HTTPException(status_code=401, detail="Incorrect email or password")
        if not verify_password(form_data.password, db_users.password):
            raise HTTPException(status_code=401, detail="Incorrect email or password")
